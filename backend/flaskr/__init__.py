...
def create_app(test_config=None):
  ...
  def get_questions():
    ...
    if page < 1: #incase somehow the user input a negative or a zero page number
      page = 1
    start = (page-1)*10
    end = start + 10
    ...
    data = {
      "success":True,
      "questions":questions,
      "length":len(Question.query.all()) #show how many questions are there in the db
    }
    return jsonify(data)
  ...
  def add_question():
    ...
    try:
      ...
      if data["answer"] is None or data["category"] is None or data["difficulty"] is None or data["question"] is None:
        abort(422) 
      ...
      return jsonify({
        ...
        "question_added":q.format() #showing what question had been added
      })
    except:
      db.session.rollback()
      abort(422)
  ...
  def search_questions():
    try:
      searchTerm = request.json.get("searchTerm","") #If there isn't a search phrase this will output all the questions
      questions = Question.query.filter( Question.question.ilike(f"%{searchTerm}%") | Question.answer.ilike(f"%{searchTerm}%") ).all()
      questions = [q.format() for q in questions]
    except:
      abort(500)
  ...
  def send_quiz_question():
    ...
    if quiz_category != 0:
      question = list( filter( lambda x: x.id not in previous_questions , Question.query.filter_by(category=quiz_category) ) )
    else:
      question = list( filter( lambda x: x.id not in previous_questions , Question.query.all() ) )
